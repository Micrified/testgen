...
func main () {
	...
	for trial := start_n_trials; trial < end_n_trials; trial++ {
		...
		for length := start_chain_length; length <= end_chain_length; length++ {
			...
			name_ppe := fmt.Sprintf("test_c%d_t%d_ppe", length, trial)
			name_std := fmt.Sprintf("test_c%d_t%d_std", length, trial)
			...
			err = maketest.Maketest(name_ppe, path, ros_gen_rules, true,
				work_ts, environment)
			if nil != err {
				panic(err)
			}
			...
			// TODO: execute test
			err = maketest.Maketest(name_std, path, ros_gen_rules, true,
				work_ts, environment)
			if nil != err {
				panic(err)
			}
